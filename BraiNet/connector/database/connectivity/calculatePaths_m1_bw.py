...
for dataset in [2]:
	structs_1 = Structure.objects.filter(dataset_id=dataset).order_by('id').reverse()
	structs_2 = Structure.objects.filter(dataset_id=dataset)
	# loop through all pairs of structures that are not ancestors / descendants and compute 
	# paths
	for s1 in structs_1.iterator():
		for s2 in structs_2.iterator():
